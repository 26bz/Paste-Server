...
export default eventHandler(async (event) => {
  ...
  const metadataList = `
    <ul class="list-group list-group-flush">
      ...
      ${paste.source ? `<li class="list-group-item bg-transparent text-light border-secondary"><strong>Source:</strong> ${escapeHtml(paste.source)}</li>` : ""}
      <li class="list-group-item bg-transparent text-light border-secondary">
    ...
    <div class="card shadow-sm p-4 mb-4">
      <div class="d-flex justify-content-between flex-wrap gap-2 align-items-start">
        ...
        <div class="d-flex gap-2">
          ...
          <button class="btn btn-outline-info btn-sm" id="copy-link" data-url="${escapeHtml(`https://${host}/p/${paste.id}`)}">Copy link</button>
        </div>
      </div>
    </div>
    <div class="row g-4">
      ...
      <div class="col-12 col-lg-8">
        <div class="card shadow-sm p-4">
          ...
          <div class="log-viewer" id="log" data-raw="${escapeHtml(
            paste.content,
          )}">
  ...
  const inlineScript = String.raw`
    (() => {
      ...
      copyLogBtn?.addEventListener("click", async () => {
        ...
        const rawLog = logEl?.getAttribute("data-raw") ?? logEl?.textContent ?? "";
        try {
...
const renderLogLines = (content: string) => {
  ...
  return normalized
    .map((line, index) => {
      ...
      return `<div class="log-line"><span class="log-line-number">${
        index + 1
      }</span><span class="log-line-text">${safeLine}</span></div>`;
    })
    .join("");
};
